...
using MFA.OHIO.BACK.Application.UseCases.Token.Command.ValidateToken;
...
namespace MFA.OHIO.BACK.Api.Controllers
{
    ...
    public class TokenController : ControllerBase
    {
        ...
        [HttpPost("validate", Name = "Validar Token")]
        public async Task<IActionResult> ValidateToken(ValidateTokenRequestDTO validateTokenRequest)
        {
            ResponseDTO responseDTO = await _mediator.Send(new ValidateTokenCommand(validateTokenRequest));
            return StatusCode(responseDTO.statusCode, new { detail = responseDTO.detail });
        }
    }
}
